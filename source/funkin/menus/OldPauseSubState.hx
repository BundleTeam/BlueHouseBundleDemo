...
class OldPauseSubState extends MusicBeatSubstate
{
	...
	public function new(x:Float, y:Float)
	{
		...
		practiceText = new FlxText(20, 15 + 101, 0, "NO FAIL", 32);
		practiceText.scrollFactor.set();
		practiceText.setFormat(Paths.font('vcr.ttf'), 32);
		practiceText.x = FlxG.width - (practiceText.width + 20);
		practiceText.updateHitbox();
		practiceText.visible = PlayState.instance.noFail;
		add(practiceText);
		...
		if (accepted && (cantUnpause <= 0 || !ClientPrefs.controllerMode))
		{
			...
			switch (daSelected)
			{
				...
				case 'Toggle Practice Mode':
					PlayState.instance.noFail = !PlayState.instance.noFail;
					PlayState.changedDifficulty = true;
					practiceText.visible = PlayState.instance.noFail;
				case "Restart Song":
					restartSong();
	...
	function regenMenu():Void
	{
		...
		for (i in 0...menuItems.length)
		{
			...
			if (menuItems[i] == 'Skip Time')
			{
				...
				skipTimeText.setFormat(Paths.font("vcr.ttf"), 64, FlxColor.WHITE, CENTER, QualityPrefs.textShadow(), FlxColor.BLACK);
