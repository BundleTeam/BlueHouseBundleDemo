...
#if (cpp && desktop) import funkin.system.Discord; #end
...
import funkin.menus.options.CheckboxThingie;
...
class GameplayChangersSubstate extends MusicBeatSubstate
{
	...
	private var curSelected:Int32 = 0;
	...
	function getOptions()
	{
		...
		var option:GameplayOption = new GameplayOption('Playback Rate', 'playbackspeed', 'float', 1);
		...
		var option:GameplayOption = new GameplayOption('Record Mode', 'recordmode', 'bool', false);
		optionsArray.push(option);

		var option:GameplayOption = new GameplayOption('Taunt Mode', 'tauntmode', 'bool', false);
		optionsArray.push(option);
	}
							...
							switch (curOption.type)
							{
								case 'int' | 'float' | 'percent':
									...
									switch (curOption.type)
									{
										...
										case 'float' | 'percent':
											holdValue = FNKMath.roundToMultiple(holdValue, curOption.changeValue);
								...
								case 'string':
									...
									// Funkin.log(curOption.options[num]);
							}
	...
	function clearHold()
	{
		...
		{
			case 'float' | 'percent':
				curOption.setValue(FlxMath.roundDecimal(FNKMath.roundToMultiple(holdValue, curOption.changeValue), curOption.decimals));
				updateTextFrom(curOption);
				curOption.change();
		}
		holdTime = 0;
