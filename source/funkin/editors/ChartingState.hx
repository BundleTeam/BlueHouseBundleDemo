...
import funkin.gameplay.SongEvents.ShotWindow;
import funkin.gameplay.SongEvents.EventList;
import funkin.utils.CoolUtil;
import funkin.music.Song;
import funkin.music.Conductor.BPMChangeEvent;
...
import funkin.gameplay.hud.HealthIcon;
import funkin.gameplay.SongEvents;
...
class ChartingState extends MusicBeatState
{
	...
	var undos = [];
	var redos = [];
	var eventStuff:Array<Array<String>> = EventList.eventDetails;
	...
	**/
	...
	override function create()
	{
		...
		for (i in 0...tipTextArray.length)
		{
			...
			tipText.y += i * 12;
			tipText.setFormat(Paths.font("vcr.ttf"), 14, FlxColor.WHITE, LEFT /*, QualityPrefs.textShadow(), FlxColor.BLACK*/);
	...
	function addSongUI():Void
	{
		...
		var characters:Array<String> = Character.getCharList();
		...
		var stages:Array<String> = SongStages.getStageList();
	...
	var meta_songTitle:FlxUIInputText;
	var meta_save:FlxUIButton;

	var meta_artist:FlxUIInputText;
	var meta_hasErect:FlxUICheckBox;
	var null_int:Null<Int> = null;

	function addMetadataUI()
	{
		var obj:Int = 0;
		var metaData:Null<String> = Song.getJson('metadata', currentSongName);

		var metaDataJson:Dynamic;
		if (metaData == null)
		{
			metaData = '{"originalMod": "","artist": "????","title": "Title","description": "Description","hasErect": false,"loaderChar": "mistashitty"}';
		}
		metaDataJson = Json.parse(metaData);

		var tab_group_metadata:FlxUI = new FlxUI(null, UI_box);
		tab_group_metadata.name = 'Metadata';

		meta_songTitle = new FlxUIInputText(15, 45 + (35 * obj), null_int, metaDataJson.title);
		meta_songTitle.name = 'Song Title:';
		blockPressWhileTypingOn.push(meta_songTitle);
		tab_group_metadata.add(meta_songTitle);
		tab_group_metadata.add(new FlxText(meta_songTitle.x, meta_songTitle.y - 15, 0, meta_songTitle.name));
		obj++;

		meta_save = new FlxUIButton(200, 45, "Save Metadata", () ->
		{
			saveMetaData();
		});
		tab_group_metadata.add(meta_save);
		meta_artist = new FlxUIInputText(15, 45 + (35 * obj), null_int, metaDataJson.artist);
		meta_artist.name = 'Artist:';
		blockPressWhileTypingOn.push(meta_artist);
		tab_group_metadata.add(meta_artist);
		tab_group_metadata.add(new FlxText(meta_artist.x, meta_artist.y - 15, 0, meta_artist.name));
		obj++;

		meta_hasErect = new FlxUICheckBox(15, 45 + (35 * obj), null, null, 'Has Erect Song');
		meta_hasErect.checked = metaDataJson.hasErect;
		tab_group_metadata.add(meta_hasErect);
		obj++;
	...
	function addSectionUI():Void
	{
		...
		var pasteButton:FlxButton = new FlxButton(copyButton.x + 100, copyButton.y, "Paste Section", function()
		{
			...
			// Funkin.log('Time to add: ' + addToTime);
	...
	function addChartingUI()
	{
		...
		#if (!html5 || !mobile)
		sliderRate = new FlxUISlider(this, 'playbackSpeed', 120, 120, 0.5, 3, 150, null_int, 5, FlxColor.WHITE, FlxColor.BLACK);
	...
	function stopSong(forceStop:Bool = false)
	{
		if (FlxG.sound.music.playing || forceStop)
		{
			FlxG.sound.music.pause();
			if (vocals != null)
				vocals.pause();
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.mouse.justPressed)
		{
			...
			var overlappedANote:Bool = false;
			for (note in curRenderedNotes)
			{
				if (FlxG.mouse.overlaps(note))
				{
					overlappedANote = true;
					...
					{
						Funkin.log('tryin to delete note...');
						deleteNote(note);
					}
				}
			}
		...
		if (!blockInput)
		{
			if (FlxG.keys.justPressed.ESCAPE)
			{
				...
				FlxTransitionableState.skipNextTransIn = true;
				FlxTransitionableState.skipNextTransOut = true;
				MusicBeatState.switchState(new funkin.editors.EditorPlayState(sectionStartTime()));
			}
		...
		var playedSound:Array<Bool> = [false, false, false, false, false]; // Prevents ouchy GF sex sounds
		curRenderedNotes.forEachAlive(function(note:Note)
		{
			...
			if (curSelectedNote != null)
			{
				...
				{
					colorSine += elapsed;
					var colorVal:Float = 0.7 + FNKMath.fastSin(Math.PI * colorSine) * 0.3;
			...
			if (note.strumTime <= Conductor.songPosition)
			{
				...
				if (note.strumTime > lastConductorPos)
				{
					if (!playedSound[4])
					{
						if (note.eventName == "Shot Warning")
						{
							ShotWindow.warningSound();
							playedSound[4] = true;
						}
					}
					if (FlxG.sound.music.playing && note.noteData > -1)
					{
						...
						if (!playedSound[data])
						{
							...
							{
								var soundToPlay = ClientPrefs.hitsound;
								if (_song.player1 == 'gf')
								{ // Easter egg
									soundToPlay = 'GF_' + Std.string(data + 1);
								}

								FlxG.sound.play(Paths.hitsound(soundToPlay)).pan = note.noteData < 4 ? -0.3 : 0.3; // would be coolio
	...
	function updateWaveform()
	{
		...
		if (!FlxG.save.data.chart_waveformInst && !FlxG.save.data.chart_waveformVoices)
		{
			// Funkin.log('Epic fail on the waveform lol');
			return;
		}
	...
	function loadHealthIconFromCharacter(char:String)
	{
		...
		#if sys
		path = path.split(":")[1]; // TODO: fix shitty fix here!
		if (!FileSystem.exists(path))
		#else
		if (!OpenFlAssets.exists(path))
		#end
		{
			path = Paths.getPath('characters/' + Character.DEFAULT_CHARACTER + '.json',
	...
	function updateGrid():Void
	{
		...
		if (_song.notes[curSec].changeBPM && _song.notes[curSec].bpm > 0)
		{
			...
			// Funkin.log('BPM of this section:');
		}
		else
		{
			// get last bpm
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			if (i[3] != null && note.noteType != null && note.noteType.length > 0)
			{
				...
				daText.setFormat(Paths.font("vcr.ttf"), 24, FlxColor.WHITE, CENTER, QualityPrefs.textShadow(), FlxColor.BLACK);
				daText.xAdd = -32;
				daText.yAdd = 6;
				daText.borderSize = 1;
		...
		for (i in _song.events)
		{
			if (endThing > i[0] && i[0] >= startThing)
			{
				...
				daText.setFormat(Paths.font("vcr.ttf"), 12, FlxColor.WHITE, RIGHT, QualityPrefs.textShadow(), FlxColor.BLACK);
				daText.xAdd = -410;
				daText.borderSize = 1;
				if (note.eventLength > 1)
					daText.yAdd += 8;
				curRenderedNoteType.add(daText);
				daText.sprTracker = note;
				// Funkin.log('test: ' + i[0], 'startThing: ' + startThing, 'endThing: ' + endThing);
			}
		}
	...
	private function addNote(strum:Null<Float> = null, data:Null<Int> = null, type:Null<Int> = null):Void
	{
		...
		// Funkin.log(noteData + ', ' + noteStrum + ', ' + curSec);
		strumTimeInputText.text = '' + curSelectedNote[0];
	...
	private function saveMetaData()
	{
		stopSong(true);
		var json = {
			"title": meta_songTitle.text,
			"artist": meta_artist.text,
			"description": "description",
			"hasErect": meta_hasErect.checked,
			"bpm": Std.string(Conductor.bpm),
			"loaderChar": "mistashitty",
			"originalMod": "",
		}
