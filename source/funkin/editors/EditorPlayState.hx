...
class EditorPlayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		scoreTxt.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, CENTER, QualityPrefs.textShadow(), FlxColor.BLACK);
		...
		sectionTxt.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, CENTER, QualityPrefs.textShadow(), FlxColor.BLACK);
		sectionTxt.scrollFactor.set();
		sectionTxt.borderSize = 1.25;
		add(sectionTxt);

		beatTxt = new FlxText(10, sectionTxt.y + 30, FlxG.width - 20, "Beat: 0", 20);
		beatTxt.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, CENTER, QualityPrefs.textShadow(), FlxColor.BLACK);
		beatTxt.scrollFactor.set();
		beatTxt.borderSize = 1.25;
		add(beatTxt);

		stepTxt = new FlxText(10, beatTxt.y + 30, FlxG.width - 20, "Step: 0", 20);
		stepTxt.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, CENTER, QualityPrefs.textShadow(), FlxColor.BLACK);
	...
	private function onKeyRelease(event:KeyboardEvent):Void
	{
		...
		// Funkin.log('released: ' + controlArray);
	}
	...
	function spawnNoteSplash(x:Float, y:Float, data:Int, ?note:Note = null)
	{
		...
		var hue:Float = ClientPrefs.ui.noteHSV[data % 4][0] / 360;
		var sat:Float = ClientPrefs.ui.noteHSV[data % 4][1] / 100;
		var brt:Float = ClientPrefs.ui.noteHSV[data % 4][2] / 100;
		if (note != null)
		{
