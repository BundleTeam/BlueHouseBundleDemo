...
import funkin.gameplay.DialogueBoxPsych.DialogueFile;
import funkin.ui.Alphabet;
import funkin.gameplay.DialogueBoxPsych.DialogueCharacter;
import funkin.gameplay.DialogueBoxPsych.DialogueLine;
...
class DialogueEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		addLineText.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, LEFT, QualityPrefs.textShadow(), FlxColor.BLACK);
		addLineText.scrollFactor.set();
		add(addLineText);

		selectedText = new FlxText(10, 32, FlxG.width - 20, '', 8);
		selectedText.setFormat(Paths.font("vcr.ttf"), 24, FlxColor.WHITE, LEFT, QualityPrefs.textShadow(), FlxColor.BLACK);
		selectedText.scrollFactor.set();
		add(selectedText);

		animText = new FlxText(10, 62, FlxG.width - 20, '', 8);
		animText.setFormat(Paths.font("vcr.ttf"), 24, FlxColor.WHITE, LEFT, QualityPrefs.textShadow(), FlxColor.BLACK);
	...
	function onLoadComplete(_):Void
	{
		...
		if (fullPath != null)
		{
			...
			if (rawJson != null)
			{
				...
				if (loadedDialog.dialogue != null && loadedDialog.dialogue.length > 0) // Make sure it's really a dialogue file
				{
					...
					Funkin.log("Successfully loaded file: " + cutName);
		...
		_file = null;
		#else
		Funkin.log("File couldn't be loaded! You aren't on Desktop, are you?");
		#end
	}
	...
	function onLoadCancel(_):Void
	{
		...
		_file.removeEventListener(IOErrorEvent.IO_ERROR, onLoadError);
		_file = null;
		Funkin.log("Cancelled file loading.");
	}
	...
	function onLoadError(_):Void
	{
		...
		_file.removeEventListener(IOErrorEvent.IO_ERROR, onLoadError);
		_file = null;
		Funkin.log("Problem loading file");
	}
