...
import funkin.gameplay.BGSprite;
#if (cpp && desktop)
...
class CharacterEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		textAnim.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, QualityPrefs.textShadow(), FlxColor.BLACK);
		...
		for (i in 0...tipTextArray.length - 1)
		{
			...
			tipText.setFormat(null, 12, FlxColor.WHITE, RIGHT, QualityPrefs.textShadow(), FlxColor.BLACK);
	...
	function addAnimationsUI()
	{
		...
		var addUpdateButton:FlxButton = new FlxButton(70, animationIndicesInputText.y + 30, "Add/Update", function()
		{
			...
			genBoyOffsets();
			Funkin.log('Added/Updated animation: ' + animationInputText.text);
		});
		...
		var removeButton:FlxButton = new FlxButton(180, animationIndicesInputText.y + 30, "Remove", function()
		{
			for (anim in char.animationsArray)
			{
				if (animationInputText.text == anim.anim)
				{
					...
					reloadAnimationDropDown();
					genBoyOffsets();
					Funkin.log('Removed animation: ' + animationInputText.text);
					break;
				}
			}
		});
	...
	function genBoyOffsets():Void
	{
		...
		for (anim => offsets in char.animOffsets)
		{
			...
			text.setFormat(null, 16, FlxColor.WHITE, CENTER, QualityPrefs.textShadow(), FlxColor.BLACK);
	...
	function reloadCharacterDropDown()
	{
		...
		characterList = Character.getCharList();
