...
class Conductor
{
	...
	public static function getCrotchetAtTime(time:Float):Float32
	{
	...
	public static function beatToSeconds(beat:Float):Float32
	{
		var step:Float32 = beat * 4;
	...
	public static function getStep(time:Float):Float32
	{
		var lastChange = getBPMFromSeconds(time);
		return lastChange.stepTime + (time - lastChange.songTime) / lastChange.stepCrochet;
	}

	public static function getStepRounded(time:Float):Float32
	{
		var lastChange = getBPMFromSeconds(time);
		return lastChange.stepTime + Math.floor(time - lastChange.songTime) / lastChange.stepCrochet;
	}

	public static function getBeat(time:Float):Float32
	{
		return getStep(time) / 4;
	}

	public static function getBeatRounded(time:Float):Int32
	{
	...
	public static function mapBPMChanges(song:SwagSong):Void
	{
		...
		var curBPM:Float32 = song.bpm;
		var totalSteps:Int32 = 0;
		var totalPos:Float32 = 0;
		for (i in 0...song.notes.length)
		{
			...
			var deltaSteps:Int32 = Math.round(getSectionBeats(song, i) * 4);
			totalSteps += deltaSteps;
			totalPos += ((60 / curBPM) * 1000 / 4) * deltaSteps;
		}
		Funkin.log("new BPM map BUDDY " + bpmChangeMap);
	}

	static function getSectionBeats(song:SwagSong, section:Int):Null<Float32>
	{
	...
	inline public static function calculateCrochet(bpm:Float):Float32
	{
		return (60 / bpm) * 1000;
	}
...
class Rating
{
	...
	public var resetsCombo:Bool = true;
