...
class Vaporwave extends FlxShaderToyHack
{
	...
	public function new()
	{
		...
		float edgeMin(float dx,vec2 da, vec2 db,vec2 uv){
			...
			vec3 c = fract((ceil(vec3(uv,uv.x+uv.y)))*(vec3(0,1,2)+0.61803398875));

			float a1 = vec4(0).x>.6?.15:1.;
			float a2 = vec4(0).x>.6?.15:1.;
			float a3 = vec4(0).x>.6?.15:1.;
		...
		vec3 gsky(vec3 rd,vec3 ld,bool mask){
			...
			vec3 back = vec3(.4,.1,.7)*(1.-.5*vec4(0).x
